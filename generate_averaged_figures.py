...
def load_training_curves(experiment_dir: Path) -> TrainingCurveData:
    ...
    if not csv_path.exists():
        raise FileNotFoundError(f"training_results.csv not found in {experiment_dir}")
    ...
    # Check for per-class metrics (make them optional for backward compatibility)
    ...
    if num_classes > 0:
        ...
        # No per-class metrics - create empty arrays (will be handled gracefully by plotting code)
        logging.warning(f"No per-class metrics found in {csv_path}, using total metrics only")
        # Create dummy single-class arrays for compatibility
        num_classes = 1
...
def load_nc_metrics(experiment_dir: Path) -> Optional[NCMetricsData]:
    ...
    try:
        ...
        nc2_cv_means_values = df['NC2_CV_means'].values
        nc2_cv_cls_values = df['NC2_CV_cls'].values
        nc2_std_means_values = df['NC2_Std_means'].values
        nc2_std_cls_values = df['NC2_Std_cls'].values
        nc2_mean_means_values = df['NC2_Mean_means'].values
        nc2_mean_cls_values = df['NC2_Mean_cls'].values
        nc2_target_values = df['NC2_Target'].values
...
def plot_averaged_nc_metrics(nc_data: List[NCMetricsData], output_dir: Path, config: str):
    ...
    # Save plot
    output_path = output_dir / f'nc_metrics_paper_style_config_{config}.png'
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    
    logging.info(f"Saved averaged NC metrics to {output_path}")
...
def process_single_config(base_path: Path, mode: str, config: str, cluster: str, 
    ...
    config_name = f"nc_config_{config}"
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        choices=['nc_with_bumps_TPT', 'nc_without_bumps_TPT', 'nc_without_dynamics'],
