...
def create_output_directory(config: ExperimentConfig) -> Path:
    ...
    output_dir = Path(config.output.output_dir) / config.get_experiment_id()
...
def generate_and_prepare_data(config: ExperimentConfig):
    ...
    test_seed = config.data.random_seed + 1 if config.data.random_seed is not None else 1
...
def create_and_train_model(
    ...
    rng = random.PRNGKey(config.random_seed or 0)
    ...
    n_epochs = config.training.total_steps // config.training.n_steps_per_epoch
    
    with tqdm(total=config.training.total_steps, desc="Training") as pbar:
        for epoch in range(n_epochs):
            for step in range(config.training.n_steps_per_epoch):
                t = epoch * config.training.n_steps_per_epoch + step
                ...
                # Track metrics
                if t % config.training.validation_interval == 0:
                    current_metrics = metrics_tracker.update(
                        classifier.model[1], params, X_train, Y_train, X_test, Y_test, grads
                    )
...
def main():
    ...
    try:
        ...
        # Train model
        classifier, train_losses, train_accuracies, test_losses, test_accuracies = create_and_train_model(
        ...
        create_visualizations(
            ...
            train_losses, train_accuracies, test_losses, test_accuracies, output_dir
        )
        
        # Save results
        save_results(
            config, classifier, train_losses, train_accuracies,
            test_losses, test_accuracies, output_dir
        )
