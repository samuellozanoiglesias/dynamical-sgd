...
class SpiralClassifier:
    ...
    def calculate_l2_distance(self, weights_a, weights_b):
        ...
        return total_squared_diff #jnp.sqrt(total_squared_diff)
    ...
    def train(self, X_train, Y_train, X_test, Y_test, T, w_max, total_steps, n_steps_per_epoch, batch_size):
        ...
        plt.legend(handles=legend_elements, fontsize=12, loc='upper right', frameon=True, shadow=True,)
        ...
        with open(f'train_losses.pkl', 'wb') as f:
            pickle.dump(losses, f)

        with open(f'test_losses.pkl', 'wb') as f:
        ...
        with open(f'train_accuracies.pkl', 'wb') as f:
            pickle.dump(accuracies, f)
        with open(f'test_accuracies.pkl', 'wb') as f:
...
T = 5000
num_classes = 3
steps = np.arange(0, 6*T, 100)
classifier = SpiralClassifier(num_classes=num_classes, T=T)
colors = ['red', 'blue', 'gold']
plt.figure(figsize=(30, 8))
for w_max, style in [(10, 'dashed'), (70, 'solid')]:
    for i in range(num_classes):
        weights = []
        for t in steps:
            class_focus = int((t // T) % num_classes)
            weight = classifier.c_fn(t % T, class_focus, w_max, T)
            weights.append(weight[i])
        plt.plot(steps, weights, linestyle=style, color=colors[i], label=f'Class {i+1}', lw=4,)

plt.title('Dynamic Focus', fontsize=18)
plt.xlabel('Steps', fontsize=14)
plt.ylabel('Class Proportion', fontsize=14)
plt.grid(True, which='both', linestyle='--', linewidth=1)
plt.xticks(fontsize=12)
plt.yticks(fontsize=12)
custom_lines = [
    Line2D([0], [0], color='black', lw=2, linestyle='solid', label='w = 70'),
    Line2D([0], [0], color='black', lw=2, linestyle='dashed', label='w = 10'),
]
plt.legend(handles=custom_lines + [Line2D([0], [0], color=colors[i], lw=4, label=f'Class {i+1}') for i in range(num_classes)],
           fontsize=14, title_fontsize=14, loc='upper right')
plt.show()

"""# Selected samples for each step"""
...
class_indices = [np.where(Y_train.argmax(axis=1) == i)[0] for i in range(num_classes)]
train_colors = ['red', 'blue', 'gold']
train_labels = np.array(Y_train.argmax(axis=1))
...
legend_elements = [
    Line2D([0], [0], marker='o', color='w', markerfacecolor='white', markeredgecolor='black', markersize=10, label='Selected Training Data'),
]

for t in range(0, 2*steps, 50):
    print("\nstep:", t)
    class_focus = int((t // T) % num_classes)
    current_weights = classifier.c_fn(t % T, class_focus, w_max, T)
    class_counts = (current_weights * batch_size).astype(int)

    rng, key = random.split(rng)
    X_batch, Y_batch = classifier.sample_by_class(X_train, Y_train, class_counts, num_classes, key, class_indices)
    ...
    plt.xticks([])
    plt.yticks([])
    plt.title('Selected samples for each step', fontsize=16)
    plt.legend(handles=legend_elements, fontsize=12, loc='upper right', frameon=True, shadow=False)

    plt.scatter(X_train[:, 0], X_train[:, 1],
                c=[train_colors[label] for label in train_labels],
                s=60, edgecolor='none', marker='o', alpha=0.5)

    plt.scatter(X_batch[:, 0], X_batch[:, 1],
                c=[train_colors[label] for label in Y_batch.argmax(axis=1)],
...
classifier = SpiralClassifier(points_per_class=100, num_classes=3, nn_width=50, learning_rate=1, real_time_visualization=True, vis_step_interval=100, track_periodic_weight_diff=True, T=500, track_weight_diff=True, weight_diff_step_interval = 1, l2_reg = 0., label = "testing", show_markers = True)#0.5*1e-3)
X_train, Y_train = classifier.make_dataset(seed = 0)
...
n_steps_per_epoch = 1
batch_size = 50
classifier.train(X_train, Y_train, X_test, Y_test, T = T, w_max = 70, total_steps = total_steps, n_steps_per_epoch = n_steps_per_epoch, batch_size= batch_size)
...
axes[0].plot(test_losses, ".", ms=0.5, alpha=0.5, color = "red", label='Test Loss',)
axes[0].plot(train_losses, ".", ms=0.5, alpha=0.5, color = "black", label='Training Loss',)
...
axes[1].plot(test_accuracies, ".", ms=0.5, alpha=0.5, color='red', label='Test Accuracy',)
axes[1].plot(train_accuracies, ".", ms=0.5, alpha=0.5, color='black', label='Training Accuracy',)
...
axes[0].plot(test_losses, ".", ms=0.5, alpha=0.5, color = "red", label='Test Loss',)
axes[0].plot(train_losses, ".", ms=0.5, alpha=0.5, color = "black", label='Training Loss',)
axes[0].set_xlim([10*5000, 15*5000])
axes[0].set_title('Learning Curve')
axes[0].axvline(x=11*5000, color='black', linestyle='--',)
axes[0].legend(fontsize=12, markerscale=15)
axes[0].set_yscale('log')

axes[1].plot(test_accuracies, ".", ms=0.5, alpha=0.5, color='red', label='Test Accuracy',)
axes[1].plot(train_accuracies, ".", ms=0.5, alpha=0.5, color='black', label='Training Accuracy',)
axes[1].set_xlim([10*5000, 15*5000])
axes[1].set_title('Accuracy Curve')
axes[1].axvline(x=11*5000, color='black', linestyle='--',)
axes[0].legend(fontsize=12, markerscale=15)
...
n_steps_per_epoch = 1
batch_size = 200
classifier.train(X_train, Y_train, X_test, Y_test, T = T, w_max = 150, total_steps = total_steps, n_steps_per_epoch = n_steps_per_epoch, batch_size= batch_size)
...
axes[0].plot(test_losses, ".", ms=0.5, alpha=0.5, color = "red", label='Test Loss',)
axes[0].plot(train_losses, ".", ms=0.5, alpha=0.5, color = "black", label='Training Loss',)
axes[0].set_xlim([15*10*500 - 5*500, 15*10*500])
axes[0].set_title('Learning Curve')
axes[0].axvline(x=15*10*500 - 4*500, color='black', linestyle='--',)
axes[0].legend(fontsize=12, markerscale=15)
axes[0].set_yscale('log')

axes[1].plot(test_accuracies, ".", ms=0.5, alpha=0.5, color='red', label='Test Accuracy',)
axes[1].plot(train_accuracies, ".", ms=0.5, alpha=0.5, color='black', label='Training Accuracy',)
axes[1].set_xlim([15*10*500 - 5*500, 15*10*500])
axes[1].set_title('Accuracy Curve')
axes[1].axvline(x=15*10*500 - 4*500, color='black', linestyle='--',)
axes[1].legend(fontsize=12, markerscale=15)
