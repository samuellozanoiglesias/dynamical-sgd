...
Configure your datasets in the DATASETS dict below:
    ...
    DATASETS = {
        "Always Bumps":       "/data/samuel_lozano/dynamical-sgd/without_bumps/spiral-no_bumps/",
        "Bumps before TPT":    "/data/samuel_lozano/dynamical-sgd/without_bumps/dartboard-no_bumps/",
        "No bumps":        "/data/samuel_lozano/dynamical-sgd/without_bumps/rings-no_bumps/",
    }
...
Five PNGs are written to OUTPUT_DIR (one line per dataset in every panel):
    ...
    NN_comparison.png                    (1x2)
    projected_nc.png                     (2x2)  <-- new

Usage
-----
    nohup python BUMPS_analysis.py --dataset_type spiral > log.out 2>&1 &
    python BUMPS_analysis.py --output ./plots --smooth 25
...
def plot_cylinder_hyperplanes(groups, output_dir, color_map, smooth_window):
    ...
    plot_single_metric(axes[0], groups, "hyperplanes", 
                        ...
                        "Angle to boundary (degrees)", "CM-alignment cosine (boundary vs $\\Delta\\mu$)",
                        color_map, smooth_window)
...
def main():
    ...
    parser.add_argument("--dataset_type", default=DATASET_TYPE, help="Dataset type to include in the analysis.")
    ...
    DATASETS = {
        "Always bumps": f"{ROOT_DIR}/with_bumps/{dataset_type}-always_bumps/",
        "Bumps before TPT": f"{ROOT_DIR}/with_bumps_before_tpt/{dataset_type}-bumps/",
        "No bumps": f"{ROOT_DIR}/without_bumps/{dataset_type}-no_bumps/",
        }
    ...
    output_dir = args.output if args.output else f"{ROOT_DIR}/plots_across_bumps/{dataset_type}"
